...
def calculate_adx(high: List[float], low: List[float], close: List[float], period: int = 14) -> float:
    ...
    for i in range(1, len(dx) if isinstance(dx, list) else 1):
        adx_values.append((adx_values[-1] * (period - 1) + dx if isinstance(dx, (int, float)) else dx) / period)

    return adx_values[-1] if adx_values else 0.0


def calculate_trend(prices: List[float], short_period: int = 10, long_period: int = 20) -> Dict[str, Any]:
    """计算趋势方向和强度"""
