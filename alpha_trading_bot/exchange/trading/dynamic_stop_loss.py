...
class DynamicStopLoss:
    ...
    def calculate_stop_loss(
        ...
        try:
            ...
            final_stop_loss = self._calculate_final_stop_loss(
                entry_price, current_price, atr_based_stop,
                risk_adjusted_stop, price_position_stop,
            ...
            return {
                ...
                'stop_loss_pct': abs((final_stop_loss - entry_price) / entry_price),
                'atr_based_pct': atr_based_stop,
                'risk_adjusted_pct': risk_adjusted_stop,
                'price_position_pct': price_position_stop,
    ...
    def _calculate_atr_stop_loss(
        ...
        logger.info(f"ATR止损计算: 倍数={atr_multiplier}, 距离={atr_stop_distance:.2%}, "
    ...
    def _adjust_for_account_risk(
        ...
        logger.info(f"风险调整止损: 账户风险={account_risk_pct:.1%}, "
    ...
    def _adjust_for_price_position(
        ...
        if price_change_pct > 0:
            ...
            if price_change_pct > 0.01:
                # 每盈利1%，允许收紧止损0.2%
                tighten_pct = min(price_change_pct * 0.2, 0.01)  # 最大收紧1%
                return -tighten_pct  # 负值表示收紧
    ...
    def _calculate_final_stop_loss(
        ...
        if position_side == 'long':
            # 考虑价格位置调整
            if position_stop < 0:  # 需要收紧止损
                effective_stop = max(atr_stop, risk_stop) + position_stop
            else:
                effective_stop = max(atr_stop, risk_stop)

            # 计算止损价格
            stop_loss_price = entry_price * (1 - effective_stop)
        ...
        logger.info(f"最终止损计算: 入场价={entry_price:.2f}, "
                   f"ATR止损={atr_stop:.2%}, 风险止损={risk_stop:.2%}, "
                   f"位置调整={position_stop:.2%}, 最终止损={effective_stop:.2%}")
        ...
        try:
            if position_side == 'long':
                ...
                if new_trailing_stop > current_stop:
                    logger.info(f"追踪止损更新: {current_stop:.2f} → {new_trailing_stop:.2f} "
    ...
    def get_volatility_regime(self, atr_percent: float, price_volatility_24h: float) -> str:
        """确定当前波动率制度"""
        # ATR百分比判断
        if atr_percent < 0.2:
            regime = 'low'
