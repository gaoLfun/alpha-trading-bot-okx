...
class DynamicPositionSizing:
    ...
    def calculate_position_size(
        ...
        try:
            ...
            final_position = self._apply_risk_factors(
                signal_adjusted, market_volatility, risk_level,
                current_price, atr_14
            )

            # 6. 计算实际交易数量
            position_size_info = self._calculate_contract_size(
                final_position, current_price, symbol
            )

            # 7. 生成详细报告
            position_report = self._generate_position_report(
                account_balance, base_position, kelly_position,
                volatility_adjusted, signal_adjusted, final_position,
                position_size_info, symbol, position_side
            )
    ...
    def _calculate_base_position(
        ...
        logger.info(f"基础仓位计算 - 风险等级: {risk_level}, 上限: {risk_limit:.1%}, "
    ...
    def _calculate_kelly_position(
        ...
        try:
            ...
            logger.info(f"凯利公式计算 - 胜率: {win_rate:.1%}, 平均盈利: {avg_win:.2%}, "
    ...
    def _adjust_for_volatility(
        ...
        logger.info(f"波动率调整 - 市场波动率: {market_volatility}, 调整系数: {vol_multiplier:.2f}, "
    ...
    def _adjust_for_signal_quality(
        ...
        logger.info(f"信号质量调整 - 信号强度: {signal_strength:.2f}, 置信度: {confidence:.2f}, "
    ...
    def _apply_risk_factors(
        ...
        # 4. 风险等级限制
        risk_limit = self.RISK_POSITION_LIMITS.get(risk_level, 0.20)
        max_position_value = config.get('account_balance', 10000) * risk_limit
        ...
        if risk_factors:
            ...
            logger.info(f"综合风险因子调整 - 风险因子: {risk_factors}, 平均: {final_risk_factor:.2f}")
        else:
    ...
    def _calculate_contract_size(
        ...
        try:
            ...
            return {
                ...
                'contracts_risk_pct': actual_position_value / config.get('account_balance', 10000)
            }
    ...
    def _generate_position_report(
        ...
        report = {
            ...
            # 建议
            'recommendation': self._get_position_recommendation(contract_info['contracts_risk_pct'])
        }
    ...
    def update_performance_metrics(
        ...
        self.performance_metrics.update({
            ...
            'avg_win': avg_win,
            'avg_loss': avg_loss,
            'sharpe_ratio': sharpe_ratio or self.performance_metrics['sharpe_ratio'],
            'max_drawdown': max_drawdown or self.performance_metrics['max_drawdown']
        })

        logger.info(f"绩效指标已更新 - 胜率: {win_rate:.1%}, 平均盈利: {avg_win:.2%}, "
                   f"平均亏损: {avg_loss:.2%}")
    ...
    def calculate_position_size_for_recovery(
        ...
        try:
            ...
            # 计算所需仓位
            avg_return = (self.performance_metrics['avg_win'] * success_prob -
                         self.performance_metrics['avg_loss'] * (1 - success_prob))
            ...
            logger.info(f"恢复性仓位计算 - 当前回撤: {current_drawdown:.1%}, "
