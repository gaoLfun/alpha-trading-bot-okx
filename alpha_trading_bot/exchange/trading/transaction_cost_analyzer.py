...
class TransactionCostAnalyzer:
    ...
    def _get_fee_rates(self, account_tier: str) -> Dict[str, float]:
        """获取当前账户等级的费率"""
        maker_fee = self.OKX_FEE_RATES['maker'].get(account_tier, self.OKX_FEE_RATES['maker']['regular'])
        taker_fee = self.OKX_FEE_RATES['taker'].get(account_tier, self.OKX_FEE_RATES['taker']['regular'])
    ...
    def calculate_transaction_cost(
        ...
        try:
            ...
            logger.info(f"交易成本计算 - 符号: {symbol}, 方向: {side}, 数量: {quantity}, "
            ...
            return TransactionCost(
                ...
                expected_price=expected_price,
                actual_price=actual_price,
                total_cost=0.01 * notional_value,  # 默认1%成本
    ...
    def analyze_execution_quality(
        ...
        try:
            ...
            total_value = sum(trade.get('amount', 0) * trade.get('price', 0) for trade in executed_trades)
            average_executed_price = total_value / total_quantity if total_quantity > 0 else requested_price
            ...
            if orderbook_data:
                ...
                mid_price = (best_bid + best_ask) / 2 if best_bid > 0 and best_ask > 0 else requested_price
            else:
            ...
            # 计算价格改善
            if side == 'buy':
                price_improvement = (requested_price - average_executed_price) / requested_price
            else:  # sell
                price_improvement = (average_executed_price - requested_price) / requested_price
            ...
            # 计算执行时间
            if executed_trades:
                first_trade_time = datetime.fromisoformat(executed_trades[0].get('timestamp', ''))
                last_trade_time = datetime.fromisoformat(executed_trades[-1].get('timestamp', ''))
            ...
            logger.info(f"执行质量分析 - 订单: {order_id}, 成交率: {fill_rate:.1%}, "
    ...
    def _calculate_execution_quality_score(self, fill_rate: float, slippage_bps: float, execution_time: float) -> float:
        """计算执行质量评分"""
        try:
            # 成交率评分 (0-40分)
            if fill_rate >= self.EXECUTION_QUALITY_BENCHMARKS['fill_rate']['excellent']:
                ...
                fill_score = 30
            elif fill_rate >= self.EXECUTION_QUALITY_BENCHMARKS['fill_rate']['acceptable']:
                fill_score = 20
            else:
                fill_score = max(0, fill_rate * 40)

            # 滑点评分 (0-30分)
            if slippage_bps <= self.EXECUTION_QUALITY_BENCHMARKS['slippage_bps']['excellent']:
                slippage_score = 30
            elif slippage_bps <= self.EXECUTION_QUALITY_BENCHMARKS['slippage_bps']['good']:
                slippage_score = 20
            elif slippage_bps <= self.EXECUTION_QUALITY_BENCHMARKS['slippage_bps']['acceptable']:
                slippage_score = 10
            else:
                slippage_score = max(0, 30 - (slippage_bps - 5) * 2)

            # 执行时间评分 (0-30分)
            if execution_time <= self.EXECUTION_QUALITY_BENCHMARKS['execution_time']['excellent']:
                time_score = 30
            elif execution_time <= self.EXECUTION_QUALITY_BENCHMARKS['execution_time']['good']:
                time_score = 20
            elif execution_time <= self.EXECUTION_QUALITY_BENCHMARKS['execution_time']['acceptable']:
                time_score = 10
            else:
    ...
    def calculate_break_even_return(self, strategy_return: float, holding_period_days: int = 1) -> float:
        """
        计算盈亏平衡所需回报率
        ...
        try:
            ...
            # 总成本 = 交易成本 + 时间成本
            total_cost = avg_cost_pct + time_cost
            ...
            if strategy_return < break_even_return:
                ...
                logger.info(f"策略回报 {strategy_return:.2%} 低于盈亏平衡点 {break_even_return:.2%}，"
                           f"需要额外 {additional_return_needed:.2%} 回报")
                return additional_return_needed
            else:
                logger.info(f"策略回报 {strategy_return:.2%} 已覆盖成本 {break_even_return:.2%}")
                return 0.0
    ...
    def get_cost_analysis_report(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            # 筛选最近的交易
            recent_transactions = [
                cost for cost in self.transaction_history
                if cost.timestamp >= cutoff_date
            ]
            ...
            # 成本分析
            total_fees = sum(cost.maker_fee + cost.taker_fee for cost in recent_transactions)
            total_slippage = sum(abs(cost.slippage) * cost.actual_price * cost.quantity
            ...
            report = {
                ...
                'total_volume': sum(cost.actual_price * cost.quantity for cost in recent_transactions),
                'total_fees': total_fees,
                'total_slippage': total_slippage,
                'total_cost': total_cost,
                'average_cost_percentage': np.mean([cost.cost_percentage for cost in recent_transactions]),
                'median_cost_percentage': np.median([cost.cost_percentage for cost in recent_transactions]),
                'cost_breakdown': {
                    'fees_percentage': (total_fees / (total_fees + total_slippage)) * 100 if total_fees + total_slippage > 0 else 0,
                    'slippage_percentage': (total_slippage / (total_fees + total_slippage)) * 100 if total_fees + total_slippage > 0 else 0
                },
                'by_side': {
                    'buy': {
                        'count': len(buy_costs),
                        'avg_cost': np.mean([cost.cost_percentage for cost in buy_costs]) if buy_costs else 0,
                        'total_slippage_bps': np.mean([cost.slippage * 10000 for cost in buy_costs]) if buy_costs else 0
                    },
                    'sell': {
                        'count': len(sell_costs),
                        'avg_cost': np.mean([cost.cost_percentage for cost in sell_costs]) if sell_costs else 0,
                        'total_slippage_bps': np.mean([cost.slippage * 10000 for cost in sell_costs]) if sell_costs else 0
                    }
                },
                'by_order_type': {
                    'maker': {
                        'count': len(maker_costs),
                        'avg_fee': np.mean([cost.maker_fee for cost in maker_costs]) if maker_costs else 0
                    },
                    'taker': {
                        'count': len(taker_costs),
                        'avg_fee': np.mean([cost.taker_fee for cost in taker_costs]) if taker_costs else 0
                    }
                },
                'execution_quality': {
                ...
                'recommendations': self._generate_cost_recommendations(recent_transactions)
            }
    ...
    def _generate_cost_recommendations(self, transactions: List[TransactionCost]) -> List[str]:
        """生成成本优化建议"""
        recommendations = []
        ...
        try:
            ...
            # 手续费分析
            taker_ratio = sum(1 for cost in transactions if cost.taker_fee > 0) / len(transactions)
            if taker_ratio > 0.8:  # 80%以上都是taker
                recommendations.append("大部分订单为Taker，建议增加Maker订单比例以降低手续费")
            ...
            if recent_executions:
                slow_executions = sum(1 for exec in recent_executions
                                    if exec.execution_time > 1.0)
    ...
    def get_slippage_forecast(self, symbol: str, side: str, quantity: float,
                            current_volatility: float, order_type: str = 'market') -> float:
        """
        预测滑点
        ...
        try:
            ...
            if historical_slippage:
                base_slippage = np.percentile(historical_slippage, 75) / 10000  # 75分位数
            else:
            ...
            predicted_slippage = base_slippage * volatility_multiplier * size_multiplier * type_multiplier

            return min(predicted_slippage, 0.01)  # 最大不超过1%
