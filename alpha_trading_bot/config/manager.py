...
class ConfigManager:
    ...
    def _load_trading_config(self) -> TradingConfig:
        ...
        return TradingConfig(
            ...
            random_offset_enabled=os.getenv("RANDOM_OFFSET_ENABLED", "true").lower()
            == "true",
            random_offset_range=int(os.getenv("RANDOM_OFFSET_RANGE", "180")),
            alphapulse_only_mode=os.getenv("ALPHA_PULSE_ONLY_MODE", "false").lower()
            == "true",
            margin_mode="cross",
            position_mode="one_way",
            allow_short_selling=os.getenv("ALLOW_SHORT_SELLING", "false").lower()
            == "true",
        )
    ...
    def _load_strategy_config(self) -> StrategyConfig:
        ...
        if investment_type not in valid_types:
            logger.warning(
                f"无效的投资类型 '{investment_type}'，使用默认值 'conservative'"
            )
        ...
        logger.info(
            f"调试 - 环境变量原始值: TAKE_PROFIT_ENABLED={take_profit_env}, STOP_LOSS_ENABLED={stop_loss_env}"
        )
        ...
        normal_take_profit_percent = (
            float(os.getenv(normal_tp_key, str(take_profit_percent * 100))) / 100
        )
        normal_stop_loss_percent = (
            float(os.getenv(normal_sl_key, str(stop_loss_percent * 100))) / 100
        )
        ...
        smart_fixed_take_profit_percent = (
            float(os.getenv(smart_fixed_tp_key, str(take_profit_percent * 100))) / 100
        )
        smart_fixed_stop_loss_percent = (
            float(os.getenv(smart_fixed_sl_key, str(stop_loss_percent * 100))) / 100
        )
        ...
        try:
            smart_multi_take_profit_levels = [
                float(x.strip()) / 100 for x in smart_multi_levels_str.split(",")
            ]
            smart_multi_take_profit_ratios = [
                float(x.strip()) for x in smart_multi_ratios_str.split(",")
            ]

            # 验证比例总和为1.0
            if abs(sum(smart_multi_take_profit_ratios) - 1.0) > 0.001:
                logger.warning(
                    f"多级止盈比例总和不为1.0，当前总和: {sum(smart_multi_take_profit_ratios)}"
                )
                # 根据投资类型使用默认比例
                if investment_type == "conservative":
                    smart_multi_take_profit_ratios = [
        ...
        # 记录配置信息
        logger.info(
            f"止盈止损配置: 止盈启用={take_profit_enabled}, 止损启用={stop_loss_enabled}"
        )
        ...
        if take_profit_enabled:
            if take_profit_mode == "normal":
                ...
                logger.info(
                    f"智能模式-固定止盈: {smart_fixed_take_profit_percent * 100:.0f}%"
                )
                logger.info(
                    f"智能模式-多级止盈: {[f'{l * 100:.0f}%' for l in smart_multi_take_profit_levels]}"
                )
        ...
        if stop_loss_enabled:
            if stop_loss_mode == "normal":
                ...
                logger.info(
                    f"智能模式-固定止损: {smart_fixed_stop_loss_percent * 100:.0f}%"
                )
        ...
        return StrategyConfig(
            ...
            smart_tp_sl_enabled=os.getenv("SMART_TP_SL_ENABLED", "true").lower()
            == "true",
            limit_order_enabled=os.getenv("LIMIT_ORDER_ENABLED", "true").lower()
            == "true",
            ...
            # 自适应止损配置
            adaptive_stop_loss_enabled=os.getenv(
            ...
            # 止盈策略配置
            profit_taking_strategy="multi_level"
            if smart_multi_take_profit_levels
            else "single_level",
