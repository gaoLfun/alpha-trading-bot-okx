...
class LowPriceStrategy:
    ...
    def enhance_signal_for_low_price(self, signal: Dict[str, Any],
        ...
        try:
            ...
            logger.info(f"🎯 应用低价格位置策略 - 级别: {level}, 位置: {price_position:.1f}%")
    ...
    def _apply_extreme_low_strategy(self, signal: Dict[str, Any],
                                  market_data: Dict[str, Any]) -> Dict[str, Any]:
        """应用极低位策略（<15%）"""
        ...
        if signal.get('signal') == 'HOLD' and self._check_buy_conditions(market_data):
            ...
            enhanced['reason'] = f"🚀 极低位反弹信号（{market_data.get('composite_price_position', 0):.1f}%）- {signal.get('reason', '')}"
        elif signal.get('signal') == 'BUY':
            # 增强已有买入信号
            enhanced['confidence'] = min(1.0, signal.get('confidence', 0.5) * 1.3)
            enhanced['reason'] = f"🔥 极低位强化买入（{market_data.get('composite_price_position', 0):.1f}%）- {signal.get('reason', '')}"

        return enhanced

    def _apply_low_strategy(self, signal: Dict[str, Any],
                           market_data: Dict[str, Any]) -> Dict[str, Any]:
        """应用低位策略（15-25%）"""
        enhanced = signal.copy()
        ...
        if signal.get('signal') == 'HOLD' and self._check_buy_conditions(market_data):
            enhanced['signal'] = 'BUY'
            enhanced['confidence'] = min(1.0, signal.get('confidence', 0.5) * 1.3)
            enhanced['reason'] = f"📈 低位买入信号（{market_data.get('composite_price_position', 0):.1f}%）- {signal.get('reason', '')}"
        elif signal.get('signal') == 'BUY':
            enhanced['confidence'] = min(1.0, signal.get('confidence', 0.5) * 1.2)
            enhanced['reason'] = f"💪 低位增强买入（{market_data.get('composite_price_position', 0):.1f}%）- {signal.get('reason', '')}"

        return enhanced

    def _apply_moderate_low_strategy(self, signal: Dict[str, Any],
                                   market_data: Dict[str, Any]) -> Dict[str, Any]:
        """应用偏低策略（25-35%）"""
        enhanced = signal.copy()
        ...
        if signal.get('signal') == 'BUY':
            ...
            enhanced['reason'] = f"👀 偏低位置增强（{market_data.get('composite_price_position', 0):.1f}%）- {signal.get('reason', '')}"
