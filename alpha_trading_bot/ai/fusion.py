...
class AIFusion:
    ...
    async def fuse_signals(
        ...
        try:
            ...
            if strategy == "consensus":
                ...
                return await self._fuse_by_weighted(
                    signals, weights, fusion_providers, market_context
                )
    ...
    def _calculate_consensus(self, signals: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 计算平均置信度
            avg_confidence = (
                total_confidence / total_signals if total_signals > 0 else 0.5
            )
    ...
    def _calculate_fused_scores(
        self, signals: List[Dict[str, Any]], consensus: Dict[str, Any]
    ) -> Dict[str, float]:
        """计算融合分数"""
        try:
            ...
            for signal in signals:
                ...
                weighted_score = (
                    ...
                    * self.fusion_weights["signal_consensus"]
    ...
    def _determine_final_signal(
        self, fused_scores: Dict[str, float], signals: List[Dict[str, Any]]
    ) -> str:
        """确定最终信号"""
        try:
            # 获取最高分数的信号
            ...
            candidate_signals = [
                sig for sig, score in fused_scores.items() if score == max_score
            ]
    ...
    def _calculate_fused_confidence(
        ...
        try:
            ...
            avg_reliability = (
                total_reliability / total_signals if total_signals > 0 else 0.5
            )
    ...
    def _apply_trend_filtering(
        ...
        try:
            ...
            # 标准化趋势方向映射
            normalized_trend = trend_direction
            ...
            if trend_strength in ["strong", "extreme"]:
                # 强势下跌趋势中，抑制买入信号
                if normalized_trend == "down" and fused_scores.get("BUY", 0) > 0:
                    # 将买入信号降级为HOLD
                    buy_score = fused_scores["BUY"]
                    fused_scores["HOLD"] = (
                        fused_scores.get("HOLD", 0) + buy_score * 0.7
                    )  # 70%转为HOLD
                    fused_scores["BUY"] = buy_score * 0.3  # 保留30%的买入倾向
                    logger.warning(
                        f"趋势过滤：强势下跌趋势中抑制买入信号 (BUY: {buy_score:.2f} -> {fused_scores['BUY']:.2f})"
                    )

                # 强势上涨趋势中，抑制卖出信号
                elif normalized_trend == "up" and fused_scores.get("SELL", 0) > 0:
                    sell_score = fused_scores["SELL"]
                    fused_scores["HOLD"] = (
                        fused_scores.get("HOLD", 0) + sell_score * 0.7
                    )
                    fused_scores["SELL"] = sell_score * 0.3
                    logger.warning(
                        f"趋势过滤：强势上涨趋势中抑制卖出信号 (SELL: {sell_score:.2f} -> {fused_scores['SELL']:.2f})"
                    )
    ...
    def _apply_signal_consistency_check(
        self, fused_scores: Dict[str, float], signals: List[Dict[str, Any]]
    ) -> Dict[str, float]:
        """应用信号一致性检查 - 处理 HOLD vs BUY 冲突"""
        try:
            # 检查是否有信号冲突的情况
            buy_score = fused_scores.get("BUY", 0)
            hold_score = fused_scores.get("HOLD", 0)
            sell_score = fused_scores.get("SELL", 0)
            ...
            # 如果存在 BUY 和 HOLD 信号的冲突，且 HOLD 信号数量不少于 BUY
            if (
                buy_score > 0
                and hold_score > 0
                and signal_counts["HOLD"] >= signal_counts["BUY"]
            ):
                # 检查是否有高置信度的 HOLD 信号
                hold_confidences = [
                    s.get("confidence", 0) for s in signals if s.get("signal") == "HOLD"
                ]
                max_hold_confidence = max(hold_confidences) if hold_confidences else 0

                # 如果 HOLD 信号置信度较高（>0.7），则降低 BUY 信号权重
                if max_hold_confidence > 0.7:
                    # 将部分 BUY 分数转移到 HOLD
                    transfer_ratio = 0.4  # 转移40%的 BUY 分数到 HOLD
                    transfer_amount = buy_score * transfer_ratio
                    fused_scores["BUY"] -= transfer_amount
                    fused_scores["HOLD"] += transfer_amount

                    logger.warning(
                        f"信号一致性检查: BUY vs HOLD 冲突，HOLD 置信度较高 "
                        f"({max_hold_confidence:.2f})，转移 {transfer_amount:.2f} 分数 "
                        f"(BUY: {buy_score:.2f} -> {fused_scores['BUY']:.2f})"
                    )

            # 如果 BUY 信号明显占优且置信度很高，保持 BUY
            elif buy_score > hold_score * 1.5 and buy_score > 0.8:
                logger.info(
                    f"信号一致性检查: BUY 信号占优 ({buy_score:.2f} vs HOLD {hold_score:.2f})，保持 BUY"
                )

            return fused_scores

        except Exception as e:
            logger.error(f"信号一致性检查失败: {e}")
            return fused_scores

    def _apply_provider_bias_correction(
        self, signals: List[Dict[str, Any]], market_context: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """应用提供商偏差纠正 - 增强版"""
        try:
            trend_direction = market_context.get("trend_direction", "neutral")
            ...
            for signal in signals:
                ...
                if provider == "qwen":
                    ...
                    if (
                        ...
                        logger.warning(
                            f"QWEN偏差纠正：下跌趋势中买入信号置信度 {confidence:.2f} -> {new_confidence:.2f}"
                        )
                    ...
                    # QWEN在上涨趋势中可能过度乐观
                        ...
                        logger.warning(
                            f"QWEN偏差纠正：上涨趋势中卖出信号置信度 {confidence:.2f} -> {new_confidence:.2f}"
                        )
                ...
                # DeepSeek特定的处理（通常更保守）
                    ...
                    if (
                        trend_strength in ["strong", "extreme"]
    ...
    async def _fuse_by_consensus(
        self, signals: List[Dict[str, Any]], threshold: float
    ) -> Optional[Dict[str, Any]]:
        """共识策略：所有模型达成一致才行动"""
        try:
    ...
    async def _fuse_by_weighted(
        ...
        try:
            ...
            # 3. 应用趋势过滤（关键修复）
            if market_context:
                fused_scores = self._apply_trend_filtering(
                    fused_scores, signals, market_context
                )

            # 4. 应用信号一致性检查（新增：处理 HOLD vs BUY 冲突）
            fused_scores = self._apply_signal_consistency_check(fused_scores, signals)
            ...
            logger.info(
                f"加权融合完成: {final_signal} (置信度: {final_confidence:.2f})"
            )
            return result
