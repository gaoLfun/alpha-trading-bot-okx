...
class PromptBuilder:
    ...
    # ============== 基础阈值（放宽） ==============
    # 买入条件阈值 - 放宽以提高信号多样性
    BUY_TREND_STRENGTH = 0.10  # 大幅降低趋势强度要求（原0.15）
    BUY_RSI_THRESHOLD = 75  # 提高RSI阈值（原72），更易买入
    BUY_BB_POSITION = 75  # 放宽布林带位置要求（原70）
    BUY_ADX_THRESHOLD = 10  # 大幅降低ADX要求（原12）
...
- +10% 持仓浮盈 > 2%（已有盈利保护）
- +15% 短期上涨动量强劲（1小时涨幅 > 1%）
- -15% 趋势为 "down"  # 降低惩罚（原-20%）
- -15% RSI > 70（接近超买）  # 降低惩罚（原-20%）
- -25% 1小时跌幅 < -2.5%（暴跌中禁止买入）  # 降低惩罚（原-30%）
- -10% ATR > 5%（高波动惩罚）  # 降低惩罚（原-15%）
- 置信度范围：50%-95%
