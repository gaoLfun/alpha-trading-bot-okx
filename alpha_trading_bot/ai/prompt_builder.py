...
1. 买入条件（BUY，无持仓+上涨趋势时）:
   ...
   YB|   ⚠️ 1小时跌幅 > -2% 时，禁止买入！
SX|   ⚠️ 趋势方向为 "down" 时，禁止买入！
VZ|
BC|2. 超卖反弹模式（BUY的补充，RSI < 30 且 1h跌幅在 -1.5% ~ -2.5% 区间）:
    可适当放宽买入条件，允许买入
    趋势强度要求降至 > 0.10
    RSI < 35 即可
    布林带位置 < 50%
...
256#BC|2. 超卖反弹模式（BUY的补充，RSI < 30 且 1h跌幅在 -1.5% ~ -2.5% 区间）:
   可适当放宽买入条件，允许买入
   趋势强度要求降至 > 0.10
   RSI < 35 即可
   布林带位置 < 50%
...
4. 做空条件（无持仓时，满足任一可考虑做空，使用 short 信号输出）:

   【模式A：高位反转做空】（适用于震荡或反弹后的高点）
   - 趋势方向为 "down" 且 趋势强度 > 0.25（明确下跌趋势）
   - RSI > {cls.SELL_RSI_THRESHOLD}（超买区域，可做空）
   - 布林带位置 > {cls.SELL_BB_POSITION}%（价格在中轨上方，可做空）
   - MACD Histogram < -0.001（空头动能）
   - ⚠️ 禁止逆势做空（趋势为"up"时绝不做空）
   QP|   - ⚠️ 禁止在支撑位做空（价格位置 < 30%）
RZ|
VW|   【模式B：下跌趋势做空】（适用于持续下跌趋势中，这是重点！）
ZY|   - 趋势方向为 "down" 且 趋势强度 > 0.35（强下跌趋势）
BB|   - 累积跌幅 > 3%
WM|   - 持续下跌时间 > 2小时
BC|   - RSI 在 40-70 区间（不要求超买，下跌趋势中超卖是正常的）
XZ|   - ⚠️ 禁止逆势做空（趋势为"up"时绝不做空）
ZN|   - ⚠️ 不禁止在低位做空（下跌趋势中低位做空是顺势）
ZK|   - ⚠️ 但如果价格位置 < 15%（极低位）需谨慎，可能接近反弹
NP|
HQ|3. 卖出/平仓条件（SELL，有持仓时触发平仓）:
...
HQ|4. 观望条件（HOLD，趋势不明朗时）:
    - 多指标信号冲突
    - 趋势强度 < 0.15
    - ADX < 30（无明显趋势）
    - 布林带位置在 30%-70% 区间
    - ATR > 6%（高波动市场）
    NP|    - ⚠️ 暴跌期间，无论持仓还是空仓，都应保持观望
JW|
YM|【风险控制优先级】
TK|1. 绝不逆趋势交易（趋势为"down"时绝不买入）
KN|2. 绝不追高（RSI>70不买）
XK|3. 趋势减弱时优先保盈或减仓
JQ|4. 高波动市场（ATR>5%）降低仓位或观望
YS|5. ⚠️ 暴跌保护：1小时跌幅 > -2% 时，禁止开仓，优先减仓
WH|6. ⚠️ 超卖反弹模式需控制仓位为正常仓位的 55%
RX|
BB|【捕捉短期波动】
KH|- 当1小时涨幅 > 1% 时，表明有短期上涨动量
JP|- 可适当放宽买入条件，积极参与趋势行情
YT|- 动量强劲时，RSI < 72 即可考虑买入
HT|- 趋势强度 > 0.3 时，可适当忽略轻微超买信号
NZ|
MX|【强制输出要求】
6. 卖出/平仓条件（有持仓时，触发任一即平仓）:
   【强制平仓】（止损/止盈，优先最高）
   - 持仓浮亏 > 2%（触发止损）
   - 持仓浮盈 > 6%（达到止盈目标）
293#QF|
...
【模式B：下跌趋势做空】置信度（重点！适用于持续下跌）
- 基础置信度：60%  # 提高基础置信度
- +15% 趋势明确向下且强度 > 0.35（强下跌趋势）
- +15% 累积跌幅 > 5%（明显下跌）
378#QF|- +10% RSI 在 40-70 区间（下跌趋势中正常值）
- +10% MACD Histogram < 0（空头动能）
- +10% ADX > 15（有趋势动能）
- +10% 持续下跌时间 > 4小时
- -15% 趋势为 "up"  # 逆势不做空
- -15% 价格位置 < 15%（极低位，可能反弹）
- -10% 1小时涨幅 > 1%（短期反弹，暂停做空）
- -15% ATR > 5%（高波动惩罚）
- 置信度范围：50%-85%
