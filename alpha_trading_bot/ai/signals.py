...
class SignalGenerator(BaseComponent):
    ...
    async def generate_trading_signals(
        self, market_data: Dict[str, Any], ai_signals: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """生成交易信号"""
        try:
            signals = []
            ...
            for ai_signal in ai_signals:
                if (
                    ...
                    trading_signal = await self._convert_ai_signal(
    ...
    async def _convert_ai_signal(
        self, ai_signal: Dict[str, Any], market_data: Dict[str, Any]
        ...
        try:
            ...
            if crash_events:
                ...
                for event in crash_events:
                    logger.info(
                        f"  - {event.timeframe}: {event.price_change * 100:.2f}% ({event.level.value})"
                    )
            else:
    ...
    def get_signal_stats(self) -> Dict[str, Any]:
        ...
        avg_confidence = (
            sum(s.get("confidence", 0) for s in self.signal_history) / total_signals
        )
...
# 向后兼容的函数
async def generate_enhanced_fallback_signal(
    market_data: Dict[str, Any], signal_history=None
) -> Optional[Dict[str, Any]]:
    """生成增强的回退信号（向后兼容）"""
    try:
