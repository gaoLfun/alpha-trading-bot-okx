...
from alpha_trading_bot.api import (
    create_bot,
    start_bot,
    stop_bot,
    get_bot_status,
...
async def clear_bots_cache():
    """每个测试前清理bots缓存"""
    from alpha_trading_bot.api.bot_api import _bots
...
class TestBotAPI:
    ...
    async def test_create_bot_success(self):
        ...
        with patch.object(TradingBot, "initialize", return_value=True):
            bot = await create_bot(
                bot_id="test-bot",
                name="Test Bot",
                config={"max_position_size": 0.01, "leverage": 10, "test_mode": True},
            )
    ...
    async def test_start_bot_success(self):
        ...
        with patch.object(TradingBot, "initialize", return_value=True):
            ...
            # Since initialize is mocked, manually set _initialized
        ...
        with patch.object(TradingBot, "initialize", return_value=True):
            ...
            with patch.object(bot, "stop", new_callable=AsyncMock):
                with patch.object(bot, "cleanup", new_callable=AsyncMock):
                    ...
                    bot.cleanup
                    assert bot.cleanup.call_count == 1
...
class TestSyncWrapperFunctions:
    ...
    def test_create_bot_sync(self):
        ...
        with patch.object(TradingBot, "initialize", return_value=True):
            ...
            bot = create_bot_sync(bot_id="sync-test-bot", name="Sync Test Bot")
            assert isinstance(bot, TradingBot)
    ...
    def test_get_bot_status_sync(self):
        ...
        # 先创建机器人
        with patch.object(TradingBot, "initialize", return_value=True):
            from alpha_trading_bot.api.bot_api import (
                create_bot_sync,
                get_bot_status_sync,
            )
